...
#define  U32_PTR_FETCH(ptr, offset) (*(reinterpret_cast<const uint32_t*>(ptr) + (offset)))
...
namespace mnem::internal {
    namespace {
        ...
        const std::byte* avx2_main_scan(const std::byte* begin, const std::byte* end, std::span<const mnem::sig_element> sig) {
            ...
            size_t hop;
            for (auto ptr = begin; ptr < end - 95; ptr += hop) {

                //magic to boost the shit up to 40576.8 MB/s on my pc
                 /* 
                * This optimization utilizes four uint32 variables to efficiently use the 
                * four 'e' registers (eax, ebx, ecx, edx) for improved performance.
                * The prefetching method is updated from NTA to a more suitable method 
                * based on the specific requirements of the code.
                * Bitwise operations are employed to calculate the number of bytes 
                * that can be skipped, enhancing overall efficiency.
                */

                hop = 32;

                uint32_t val47 = U32_PTR_FETCH(ptr, 47);
                uint32_t val63 = U32_PTR_FETCH(ptr, 63);
                uint32_t val79 = U32_PTR_FETCH(ptr, 79);
                uint32_t val95 = U32_PTR_FETCH(ptr, 95);

                bool cond1 = (val47 > 0x16) && (val63 > 0x16);
                bool cond2 = (val79 > 0x16) && (val95 > 0x16);

                hop += 32 * (cond1 << cond2); //1 (1) or 2 (10) 

                _mm_prefetch(reinterpret_cast<const char*>(ptr + 4096), _MM_HINT_T0);
                _mm_prefetch(reinterpret_cast<const char*>(ptr + 4096), _MM_HINT_T1);
